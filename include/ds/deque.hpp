...
public:
  ...
  Iterator& operator++()
  {
    ...
    if (m_current == (*m_block)->end())
    {                                  // Se chegou ao fim do bloco
  ...
  Iterator& operator--()
  {
    if (m_current == (*m_block)->begin())
    {                                // Se chegou ao início do bloco
  ...
  bool operator<(const Iterator& other) const { return (m_block < other.m_block) or ((m_block == other.m_block) and (m_current < other.m_current)); }
  bool operator>(const Iterator& other) const { return not(*this < other); }
...
public:
  ...
  Deque(const Deque& other)
  {
    ...
    auto front_blk_index{ other.m_front_itr.m_block - other.m_mob->begin() }; // faz o calculo da distância entre o bloco
...
public:
  ...
  [[nodiscard]] std::string to_string_full() const
  {
    ...
    oss << "\n *front_ptr: " << *m_front_itr.get_current() << ", *back_ptr: " << *(m_back_itr - 1).get_current();
    oss << " e size: " << size() << "\n";
    return oss.str();
  }
